...
public class CrudHandlerV3 {
    ...
    public HttpResponseMessage handlePost(HttpRequestMessage<Optional<String>> request, Connection conn, 
        ...
        if (jsonBody == null || jsonBody.isEmpty()) {
            return ResponseUtil.validationError(request, ErrorCode.MISSING_BODY, "request_body", "リクエストボディは空にできません");
        }
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            if (affectedRows > 0) {
                ...
                throw new SQLException("挿入操作が行に影響を与えませんでした");
            }
        } catch (SQLException e) {
    ...
    public HttpResponseMessage handlePatch(HttpRequestMessage<Optional<String>> request, Connection conn, 
        ...
        if (queryParams.isEmpty()) {
            return ResponseUtil.validationError(request, ErrorCode.MISSING_FILTER, "query_parameters", "PATCHリクエストにはフィルタリングのためのクエリパラメータが必要です");
        }
    ...
    public HttpResponseMessage handleDelete(HttpRequestMessage<Optional<String>> request, Connection conn, 
        ...
        if (queryParams.isEmpty()) {
            return ResponseUtil.validationError(request, 
                ...
                "query_parameters", 
                "DELETEリクエストにはフィルタリングのためのクエリパラメータが必要です");
        }
